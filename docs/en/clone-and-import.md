...
`git clone https://github.com/ByteLegendQuest/java-symmetric-string`

A project directory named `java-symmetric-string` will be created, which can be moved wherever you like.

Open `IDEA`, `File` - `Open`, select and open the `pom.xml` in `java-symmetric-string` directory which is created just now.
